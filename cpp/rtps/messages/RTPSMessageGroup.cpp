...
/**
 * 判断待添加的数据是否超过了设定的限制。函数输入参数包括待添加数据的大小、限制的大小、已发送的数据总量和待发送的数据量。
 * 函数返回一个布尔值，如果待添加数据大小超过了限制或者加上待添加数据后会超过限制，则返回true；否则返回false。
*/
...
/**
 * 该函数是一个静态函数，其作用是将一个子消息（submsg）追加到一个完整消息（full_msg）中。
 * 函数的参数包括一个RTPSParticipantImpl类型的指针（participant），以及两个CDRMessage_t类型的指针（full_msg和submsg）。
 * 函数首先根据定义的宏HAVE_SECURITY和FASTDDS_STATISTICS来计算需要额外预留的空间大小（extra_size），然后调整full_msg的max_size，
 * 再调用CDRMessage::appendMsg函数将submsg追加到full_msg中，最后恢复full_msg的max_size并返回追加操作的结果（true或false）
*/
...
/**
 * 该函数用于比较两个远程参与者向量，分别为remote_participants1和remote_participants2。
 * 函数首先比较两个向量的大小，如果相等，则遍历remote_participants1中的每个参与者，检查其是否存在于remote_participants2中。
 * 如果每个参与者都存在于remote_participants2中，则返回true，否则返回false。如果两个向量的大小不相等，则直接返回false。
*/
...
/**
 * 该函数用于获取一个GUID_t类型的向量中的所有GuidPrefix_t类型的元素，并将它们存储在一个新的std::vector<GuidPrefix_t>类型的向量中。
*/
...
void RTPSMessageGroup::send()
{
    if (endpoint_ && sender_)
    {
        ...
        if (full_msg_->length > RTPSMESSAGE_HEADER_SIZE)
        {
            ...
            // std::cout<<"send message size: "<<msgToSend->length<<std::endl;
